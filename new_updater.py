...
def update_windows_updater():
    ...
    bat_path = os.path.join(base_path, ".ci/update_windows/update_kaonashiui.bat")

    dest_updater_path = os.path.join(top_path, "update/update.py")
    dest_bat_path = os.path.join(top_path, "update/update_kaonashiui.bat")
    dest_bat_deps_path = os.path.join(top_path, "update/update_kaonashiui_and_python_dependencies.bat")
    ...
    try:
        with open(dest_bat_deps_path, 'rb') as f:
            contents = f.read()
            contents = contents.replace(b'..\\python_embeded\\python.exe .\\update.py ..\\kaonashiUI\\', b'call update_kaonashiui.bat nopause')
