...
describe("users", () => {
	...
	function expectNoUserScreen() {
		...
		const selection = document.querySelectorAll("#kaonashi-user-selection")?.[0];
		expect(selection["style"].display).toBe("none");
		const menu = document.querySelectorAll(".kaonashi-menu")?.[0];
	...
	describe("multi-user", () => {
		...
		async function testUserScreen(onShown, users) {
			...
			expect(localStorage["kaonashi.userId"]).toBeFalsy();
			expect(localStorage["kaonashi.userName"]).toBeFalsy();
			...
			expect(document.querySelectorAll("#kaonashi-user-selection")).toHaveLength(0);
			expect(window.getComputedStyle(menu)?.display).not.toBe("none");
			...
			if (isCreate) {
				expect(api.storeUserData).toHaveBeenCalledWith("kaonashi.templates.json", null, { stringify: false });
		...
		it("allows user creation if no users", async () => {
			...
			expect(localStorage["kaonashi.userId"]).toBe("Test User!");
			expect(localStorage["kaonashi.userName"]).toBe("Test User");
		});
		it("allows user creation if no current user but other users", async () => {
			...
			expect(localStorage["kaonashi.userId"]).toBe("Test User 3!");
			expect(localStorage["kaonashi.userName"]).toBe("Test User 3");
		});
		it("allows user selection if no current user but other users", async () => {
			...
			expect(localStorage["kaonashi.userId"]).toBe("B!");
			expect(localStorage["kaonashi.userName"]).toBe("B");
		});
		it("doesnt show user screen if current user", async () => {
			const starting = start({
				...
				localStorage: {
					"kaonashi.userId": "User!",
					"kaonashi.userName": "User",
				},
			});
			await new Promise(process.nextTick); // wait for promises to resolve
		...
		it("allows user switching", async () => {
			...
			expect(app.ui.settings.settingsLookup["kaonashi.SwitchUser"]).toBeTruthy();
		});
	});
	describe("single-user", () => {
		...
		it("doesnt allow user switching", async () => {
			...
			expect(app.ui.settings.settingsLookup["kaonashi.SwitchUser"]).toBeFalsy();
		});
	});
