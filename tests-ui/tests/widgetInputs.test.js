	...
	describe("reroutes", () => {
		async function checkOutput(graph, values) {
			expect((await graph.toPrompt()).output).toStrictEqual({
				...
				7: {
					inputs: { filename_prefix: values.filename_prefix ?? "kaonashiUI", images: ["6", 0] },
		...
		it("can connect primitive via a reroute path to a widget input", async () => {
			...
			expect(filePrimitive.widgets.value.value).toBe("kaonashiUI");
			filePrimitive.widgets.value.value = "kaonashiTest";
