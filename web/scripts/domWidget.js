...
function computeSize(size) {
	...
	for (const w of this.widgets) {
		if (w.type === "converted-widget") {
			...
			let minHeight = w.options.getMinHeight?.() ?? parseInt(styles.getPropertyValue("--kaonashi-widget-min-height"));
			let maxHeight = w.options.getMaxHeight?.() ?? parseInt(styles.getPropertyValue("--kaonashi-widget-max-height"));

			let prefHeight = w.options.getHeight?.() ?? styles.getPropertyValue("--kaonashi-widget-height");
...
export function addDomClippingSetting() {
	app.ui.settings.addSetting({
		id: "kaonashi.DOMClippingEnabled",
