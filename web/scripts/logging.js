import { $el, kaonashiDialog } from "./ui.js";
import { api } from "./api.js";

$el("style", {
	textContent: `
        .kaonashi-logging-logs {
            display: grid;
            color: var(--fg-color);
            white-space: pre-wrap;
        }
        .kaonashi-logging-log {
            display: contents;
        }
        .kaonashi-logging-title {
            background: var(--tr-even-bg-color);
            font-weight: bold;
            margin-bottom: 5px;
            text-align: center;
        }
        .kaonashi-logging-log div {
...
class kaonashiLoggingDialog extends kaonashiDialog {
	constructor(logging) {
		super();
	...
	export() {
		...
		const a = $el("a", {
			href: url,
			download: `kaonashiui-logs-${Date.now()}.json`,
			style: { display: "none" },
			parent: document.body,
		});
		a.click();
	...
	show(entries) {
		...
		const headers = Object.values(cols).map((title) =>
			$el("div.kaonashi-logging-title", {
				textContent: title,
			})
		);
		const rows = entries.map((entry, i) => {
			return $el(
				"div.kaonashi-logging-log",
				{
...
export class kaonashiLogging {
	...
	constructor(app) {
		...
		this.dialog = new kaonashiLoggingDialog(this);
		this.addSetting();
		this.catchUnhandled();
		this.addInitData();
	}
	...
	addSetting() {
		const settingId = "kaonashi.Logging.Enabled";
	...
	async addInitData() {
		...
		const source = "kaonashiUI.Logging";
