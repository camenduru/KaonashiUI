...
import { kaonashiDialog } from "./dialog.js";

export class kaonashiSettingsDialog extends kaonashiDialog {
	constructor(app) {
		super();
		this.app = app;
		this.settingsValues = {};
		...
		this.element = $el(
			"dialog",
			{
				id: "kaonashi-settings-dialog",
				parent: document.body,
			},
			[
				$el("table.kaonashi-modal-content.kaonashi-table", [
					$el(
						"caption",
						{ textContent: "Settings" },
						$el("button.kaonashi-btn", {
							type: "button",
							textContent: "\u00d7",
							onclick: () => {
								this.element.close();
	...
	async setSettingValueAsync(id, value) {
		...
		localStorage["kaonashi.Settings." + id] = json; // backwards compatibility for extensions keep setting in storage
	...
	addSetting({ id, name, type, defaultValue, onChange, attrs = {}, tooltip = "", options = undefined }) {
		...
		if (value == null) {
			if (this.app.isNewUserSession) {
				...
				const localValue = localStorage["kaonashi.Settings." + id];
				if (localValue) {
		...
		this.settingsLookup[id] = {
			...
			render: () => {
				...
				const labelCell = $el("td", [
					$el("label", {
						for: htmlID,
						classList: [tooltip !== "" ? "kaonashi-tooltip-indicator" : ""],
						textContent: name,
					}),
				]);
