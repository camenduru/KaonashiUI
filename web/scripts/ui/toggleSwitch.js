...
export function toggleSwitch(name, items, { onChange } = {}) {
	...
	function updateSelected(index) {
		if (selectedIndex != null) {
			elements[selectedIndex].classList.remove("kaonashi-toggle-selected");
		}
		onChange?.({ item: items[index], prev: selectedIndex == null ? undefined : items[selectedIndex] });
		selectedIndex = index;
		elements[selectedIndex].classList.add("kaonashi-toggle-selected");
	}
	...
	const container = $el("div.kaonashi-toggle-switch", elements);
