...
export class UserSelectionScreen {
	async show(users, user) {
		...
		const userSelection = document.getElementById("kaonashi-user-selection");
		...
		return new Promise((resolve) => {
			...
			const error = userSelection.getElementsByClassName("kaonashi-user-error")[0];
			const button = userSelection.getElementsByClassName("kaonashi-user-button-next")[0];
			...
			if (user) {
				const name = localStorage["kaonashi.userName"];
				if (name) {
					input.value = name;
				}
			}
